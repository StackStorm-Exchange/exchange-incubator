...
class configureLogicalSwitch(Action):
    ...
    def run(self, nsx_mgr_ip=None, nsx_mgr_user=None, nsx_mgr_pass=None, lswitch_name=None,
            vlan=None, port_name=None, switch_name=None, hardware_gateway_name=None):
        ...
        controller = NSX(raml_file=file_path, password=nsx_mgr_pass, username=nsx_mgr_user,
                         ip=nsx_mgr_ip)
        changes = {}
        changes['pre_requisites'] = self._check_requirements(controller, lswitch_name,
                                                             hardware_gateway_name)
        changes['configure_lswitch'] = False
        if changes['pre_requisites']:
            changes['configure_lswitch'] = self._configure_lswitch(controller, lswitch_name,
                                                                   vlan, port_name, switch_name,
                                                                   hardware_gateway_name)
        else:
            self.logger.info('Pre-requisites validation failed for lswitch configuration.')
        if changes['configure_lswitch']:
            self.logger.info('closing connection to %s after configuring logical switch \
    ...
    def _configure_lswitch(self, controller, lswitch_name, vlan, port_name, switch_name,
                           hardware_gateway_name):
        '''
