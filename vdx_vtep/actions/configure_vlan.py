...
class CreateVlan(Action):
    ...
    def run(self, host=None, username=None, password=None, vlan=None, intf_desc=None):
        ...
        with pynos.device.Device(conn=conn, auth=auth) as device:
            ...
            vlan_list = self._check_vlan(vlan_id=vlan)
            ...
            if vlan_list and valid_desc:
                changes['vm_vlan'] = self._create_vlan(device, vlan_id=vlan_list, intf_desc=intf_desc, host=host)
            else:
    ...
    def _create_vlan(self, device, vlan_id, intf_desc, host):
